...
This page is what a new user needs to run it on a Mac. The design record is in the workspace's
`docs/`; the API is documented in the docstrings.
...
(the MLX oracle lives in the same repository); the product you are installing is the `nnseg`
package and command. Add `serve` to the extras for the local server, `remote` for the client:
