...
from .tasks import LAYOUTS, resolve_model_folder, weights_root
...
class WeightsStore:
    ...
    >>> WeightsStore(layout="nnunetv2").resolve(500)      # no download source; must be present
    ...
    Subclass and override :meth:`fetch` to pull from somewhere other than the layout's own
    release assets.
    """

    def __init__(self, root=None, *, layout: str = "ts", fetch: bool = True,
                 progress=None):
        self.layout = layout
    ...
    def root(self) -> Path:
        ...
        if self._root is None:
            self._root = weights_root(self.layout, None)
        return self._root
    ...
    def ensure(self, task, *, catalog=None) -> list[Path]:
        ...
        if self.layout != "ts":
            raise ModelNotFound(
                f"no download source for weights layout {self.layout!r}; place the weights under "
    ...
    def fetch(self, weights_id) -> Path:
        ...
        if self.layout != "ts":
            raise ModelNotFound(
                ...
                f"source for weights layout {self.layout!r}. Put the model folder there, pass a "
                f"path directly, or subclass WeightsStore.fetch.")
        return fetch_one(weights_id, self.root, progress=self.progress)

    # -- provenance ---------------------------------------------------------
    def describe(self) -> dict:
        return {"kind": type(self).__name__, "layout": self.layout,
                "root": str(self._root) if self._root is not None else "(default)",
                "fetch": self.fetch_enabled}

    def __repr__(self) -> str:
        root = str(self._root) if self._root is not None else "default"
        return f"{type(self).__name__}({root!r}, layout={self.layout!r}, fetch={self.fetch_enabled})"


def as_store(weights, *, layout: str = "ts") -> WeightsStore:
    """Coerce what a caller passed into a store: a store, a path, or ``None`` for the default."""
    if isinstance(weights, WeightsStore):
        return weights
    return WeightsStore(weights, layout=layout)


__all__ = ["WeightsStore", "as_store", "LAYOUTS"]
